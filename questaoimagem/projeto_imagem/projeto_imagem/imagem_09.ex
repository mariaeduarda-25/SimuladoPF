...
defmodule Imagem do
  ...
  defp gerar_imagem([largura, altura, cor_maxima, lista_subpixeis]) do
  ...
  defp decodificar_ppm(conteudo_arquivo) do
    ...
    case resultado_extracao do
      {:ok, [largura, altura, cor_maxima, lista_subpixeis]} ->
        case largura * altura * 3 == length(lista_subpixeis) do
          true ->
            img = gerar_imagem([largura, altura, cor_maxima, lista_subpixeis])
            {:ok, img}
...
## Exemplo de uso:
#case Imagem.ler("dg.ppm") do
#  {:ok, img} -> img |> Imagem.escrever("dg_copia.ppm")
#  {:error, msg} -> IO.puts("Erro: #{msg}")
#end
