...
defmodule Imagem do
  ...
  def ler(caminho_arquivo) do
    case File.read(caminho_arquivo) do
      {:ok, conteudo_arquivo} -> conteudo_arquivo |> decodificar_ppm()
      {:error, :enoent} -> {:error, "Arquivo não encontrado: #{caminho_arquivo}"}
      {:error, razao} -> {:error, "Erro ao ler arquivo '#{caminho_arquivo}': #{razao}"}
    end
  end

  @doc """
    Decodifica uma string PPM para uma struct Imagem.
    Em versão FUTURA será privada :-)

    Retorna '{:ok, imagem}'.
  """
  def decodificar_ppm(conteudo_arquivo) do
